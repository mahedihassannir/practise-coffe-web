...
const AccStatement = () => {
  ...
  const handleFromOnSubmit = (e) => {
    ...
    fetch(`https://api.ecom-bd.com/api/v1/seller/${sellerId}/withdraw_req`, {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://api.ecom-bd.com/api/v1/seller/${sellerId}/payment_history`, {
          headers: { Authorization: `Bearer ${sellerAuthToken}` }
        });
